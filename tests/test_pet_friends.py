...
def test_add_new_pet_with_valid_data(name='Miss', animal_type='cat', age='1', pet_photo='images/miss.jpg'):
    """Проверяем, что можно добавить питомца с фотографией и верными данными"""
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Барон", "кот", "2", "images/cat1.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_successful_update_self_pet_info(name='Кот', animal_type='кот', age=2):
...
def test_add_new_pet_simple_with_valid_data(name='Мурзик', animal_type='пес', age='1'):
    """Проверяем, что можно добавить питомца без фотографии с верными данными"""
...
def test_add_pet_photo_with_valid_data(pet_photo='images/miss.jpg'):
...
def test_add_new_pet_simple_with_invalid_key(name='Пес', animal_type='dog', age='2'):
...
def test_add_new_pet_with_invalid_data(name='Bob', animal_type='cat',
    ...
    assert status == 400
    # Обнаружен баг, так как принимает возраст с отрицательным значением ('-1'),
...
def test_add_pet_with_valid_data_empty_field():
    ...
    assert status == 400
    # Обнаружен баг. Сайт позволяет добавлять питомцев с отсутствием данных.
    # Фактический результат (status == 200) расходится с ожидаемым (status == 400)!!!


def test_add_new_pet_with_special_symbols_in_name(name='B@s', animal_type='cat', age='8', pet_photo='images/cat1.jpg'):
    ...
    status, result = pf.add_new_pet(auth_key, name, animal_type, age, pet_photo)
    assert status == 400
    # Обнаружен баг. Сайт позволяет добавлять питомцев, у которых в имени есть спец.символы ('B@s').
    # Фактический результат (status == 200) расходится с ожидаемым (status == 400)!!!

def test_add_new_pet_with_invalid_data(name='Мики', animal_type='мышь', age='1', pet_photo='images/miki.doc'):
    ...
    assert status == 415
    # Обнаружен баг. Сайт принимает фото питомца в формате doc-файл
